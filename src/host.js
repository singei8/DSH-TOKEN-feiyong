/*
 * DSH-TOKEN-feiyong — Host half
 * ============================================================
 * 这是 DSH「动态 Cordis 包」（dynamic package）的 Host half 半体，
 * 整个文件就是 cordis_define 参数 code.host 的**函数体本身**：
 * 开头是注释，随后直接 `return { apply(ctx) { ... } }`，没有 import/export，也不是可单独 node 运行的模块。
 *
 * 用法：把本文件内容整体作为 code.host 传给 cordis_define，再用 cordis_run 激活。
 * 平台：Host（Node 进程）— 拦截模型调用、计费、余额、落盘。
 */
...
async function resolveStorePath() {
  const settings = hostCtx === null ? undefined : hostCtx.get('settings')
  ...
  try {
    ...
    if (cut <= 0) return ''
    storeDir = documentPath.slice(0, cut)
    return joinPath(storeDir, STORE_FILE)
  } catch (error) {
    logError('store: settings.prepareDocument failed: ' + errorText(error))
    return ''
  }
}
...
async function loadStore() {
  const fs = hostCtx === null ? undefined : hostCtx.get('fs')
  if (config.persist !== true) { storeReady = true; return }
  if (fs === undefined) { storeError = 'fs 服务不可用'; logError('store: ' + storeError); return }
  storePath = await resolveStorePath()
  if (storePath.length === 0) {
    storeError = '无法定位存档路径（settings.prepareDocument 无返回）'
...
async function flushStore() {
  ...
  try {
    while (storeDirty === true) {
      ...
      const target = await fs.resolve(storePath)
      await fs.writeText(target, payload, undefined, undefined, {
...
function balanceCommand(mode) {
  if (mode === 'env') {
    return 'Invoke-RestMethod -Uri ' + shellQuote(config.balanceUrl)
      + ' -Headers @{ Authorization = ("Bearer " + $env:DSH_BALANCE_KEY) } -TimeoutSec 20 | ConvertTo-Json -Compress -Depth 6'
  }
  return '$k = (Select-String -Path (Join-Path $env:USERPROFILE \'.dsh\\.credentials.yaml\') -Pattern '
    + shellQuote(config.credentialRef + ':\\s*(\\S+)') + ').Matches[0].Groups[1].Value; '
    + 'Invoke-RestMethod -Uri ' + shellQuote(config.balanceUrl)
    + ' -Headers @{ Authorization = ("Bearer " + $k) } -TimeoutSec 20 | ConvertTo-Json -Compress -Depth 6'
}

function runShell(shell, command, env) {
  const request = { command: command, timeoutMs: 25000, stdoutMaxBytes: 65536 }
  if (env !== undefined && env !== null) request.env = env
  const spec = shell.resolve(request)
  if (config.unconfinedBalance === true) {
    spec.sandboxPolicy = { mode: 'danger-full-access', workspaceRoot: spec.workdir }
  }
  return shell.run(spec)
}

function probeText(probe) {
  if (probe === null || probe === undefined) return '无返回'
  const stdout = (probe.stdout !== undefined && probe.stdout !== null && typeof probe.stdout.text === 'string') ? probe.stdout.text.trim() : ''
  const stderr = (probe.stderr !== undefined && probe.stderr !== null && typeof probe.stderr.text === 'string') ? probe.stderr.text.trim() : ''
  if (stdout.length > 0) return 'stdout=' + stdout.slice(0, 240)
  if (stderr.length > 0) return 'stderr=' + stderr.slice(0, 240)
  return 'empty output'
}

function probeSummary(probe) {
  if (probe === null || probe === undefined) return 'no result'
  let sandbox = ''
  if (probe.sandbox !== undefined && probe.sandbox !== null) {
    sandbox = ' sandbox=' + String(probe.sandbox.mode) + '/denied=' + String(probe.sandbox.denied)
  }
  return 'exit=' + String(probe.exitCode) + sandbox + ' ' + probeText(probe)
}

function parseBalance(probe) {
  if (probe === null || probe === undefined || typeof probe !== 'object') return null
  if (probe.exitCode !== 0) return null
  const stdout = (probe.stdout !== undefined && probe.stdout !== null && typeof probe.stdout.text === 'string') ? probe.stdout.text.trim() : ''
  if (stdout.length === 0) return null
  let data = null
  try { data = JSON.parse(stdout) } catch (error) { return null }
...
async function fetchBalance() {
  ...
  const shell = hostCtx === null ? undefined : hostCtx.get('shell')
  if (shell === undefined) return failed('shell 服务不可用，无法发起带鉴权的请求')
  let key = ''
  let keySource = 'none'
  if (credentials !== undefined) {
    try {
      const resolved = await credentials.resolve(config.credentialRef)
      if (resolved !== null && resolved !== undefined && typeof resolved.value === 'string') {
        key = resolved.value
  ...
  if (key.length > 0) {
    ...
    attempts.push({ label: 'env', command: balanceCommand('env'), env: env })
  }
  attempts.push({ label: 'file', command: balanceCommand('file'), env: undefined })
  for (let index = 0; index < attempts.length; index += 1) {
    const attempt = attempts[index]
    let probe = null
    try {
      probe = await runShell(shell, attempt.command, attempt.env)
    } catch (error) {
      logError('balance[' + attempt.label + '] shell threw: ' + errorText(error))
      continue
    }
    const parsed = parseBalance(probe)
    if (parsed !== null) {
      parsed.via = attempt.label
      lastBalanceOk = true
      const fingerprint = String(parsed.currency) + ':' + String(parsed.total)
      if (lastBalanceLog !== fingerprint) {
        lastBalanceLog = fingerprint
        logInfo('balance ok via ' + attempt.label + ': ' + parsed.total + ' ' + parsed.currency + ' (keySource=' + keySource + ')')
      }
      return parsed
    }
    logError('balance[' + attempt.label + '] ' + probeSummary(probe))
  }
  return failed('余额接口未返回可用数据（详见 Host 日志 [billing] 前缀）')
}
...
return {
  apply(ctx) {
    ...
    harness.handle('billing/state', function (args) {
      return snapshot(readSessionId(args))
    })

    harness.handle('billing/save', function (args) {
    ...
    harness.handle('billing/store', function (args) {
      storeDirty = true
    ...
    harness.handle('billing/balance', function (args) {
      return kickBalance(true).then(function () {
        return snapshot(readSessionId(args))
      })
    })

    harness.handle('billing/reset', function (args) {
      ledger.length = 0
      counter = 0
      resetInto(totals)
      clearObject(byModel)
