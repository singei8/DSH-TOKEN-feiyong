...
来源 <https://docs.bigmodel.cn/cn/guide/start/pricing>。GLM **不分高峰 / 低谷**，两套单价相同：

| 档位 | 对应模型 | 输入·缓存命中 | 输入·缓存未命中 | 输出 |
| --- | --- | --- | --- | --- |
| `glm-5.3-flash` | `glm-5.3-flash`（任意 provider） | **0.23** | **0.8** | **2.8** |
| `glm-4.7` | `glm-4.7`（输入 32K–200K 档） | **0.8** | **4** | **16** |

> GLM-4.7 官方按输入长度分三档，这里取最常用的一档：输入 `[32K, 200K)` = ¥4 / ¥16 / 命中 ¥0.8。
> 另两档为 输入 `<32K` 且输出 `<0.2K` = ¥2 / ¥8 / 命中 ¥0.4、输入 `<32K` 且输出 `≥0.2K` = ¥3 / ¥14 / 命中 ¥0.6；
> 需要时在设置页**新增同名档位**即可覆盖（内置表只用作文档里没有该模型时的兜底）。
