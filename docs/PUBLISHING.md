...
**移植已完成并验证**：`node scripts/check.mjs` 202 项自检全过（真起 HTTP 服务跑通 5 条路由、
...
`node scripts/check.mjs` —— **202 项全过**：
...
1. ✅ 完成移植，`node scripts/check.mjs` 202 项全过，`main` 已推到
