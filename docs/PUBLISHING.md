...
| `tarball` 字段 | Release 资产 `dsh-token-feiyong.tgz`（**不带版本号**，最新为 v1.6.0） | ✅ 已发布 |
...
**移植已完成并验证**：`node scripts/check.mjs` 204 项自检全过（真起 HTTP 服务跑通 5 条路由、
...
`node scripts/check.mjs` —— **204 项全过**：
...
1. ✅ 完成移植，`node scripts/check.mjs` 204 项全过，`main` 已推到
