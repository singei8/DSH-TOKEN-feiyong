...
window.__ModuleLoader__.load({ id: 'dsh-token-feiyong', factory: function (require) {
  ...
  function apply(ctx) {
    ...
    ctx.effect(function () { return installStyles(CSS) })
    ...
    /** 额度窗口的中文名（对话栏徽标与设置页共用，口径按用户指定）。 */
    function quotaWindowLabel(label) {
      const key = String(label)
      if (key === '5h') return '5小时'
      if (key === 'weekly') return '周额度'
      if (key === 'monthly') return '月额度'
      if (key === 'session') return '会话'
      return key
    }

    function tierLabel(row) {
      if (row.priceMatch === 'model' || row.priceMatch === 'exact' || row.priceMatch === 'builtin') {
        return row.priceKey === undefined ? '—' : row.priceKey + (row.priceMatch === 'builtin' ? '（内置）' : '')
      }
    ...
    function useBilling(sessionId, intervalMs, onError) {
      ...
      box.onError = onError
      const pull = function () {
        return hostCall('billing/state', { sessionId: sessionId }).then(function (next) {
    ...
    function BillingMeter(props) {
      ...
      /** 套餐制（额度）口径：方舟 Agent Plan 这类不统计钱，改显示额度。 */
      const quotaMode = state.quota !== undefined && state.quota !== null && state.quota.muted === true
      ...
      if (quotaMode) {
        ...
        for (let index = 0; index < quotaRemaining.length; index += 1) {
          ...
          tipLines.push('  ' + quotaWindowLabel(item.label) + ' 已用 ' + fmtQuota(item.used) + '/' + fmtQuota(item.total)
      ...
      if (showBalance) {
        ...
        if (quotaMode === true && balanceOk && balanceKind === 'quota') {
          ...
          for (let index = 0; index < quotaPeriods.length; index += 1) {
            const period = quotaPeriods[index]
            const reset = String(period.resetAt === undefined ? '' : period.resetAt).replace('T', ' ').slice(0, 16)
            tipLines.push('  ' + quotaWindowLabel(period.label) + ' 已用 ' + Number(period.used).toFixed(2) + '/' + String(period.total)
              + '（' + Number(period.percent).toFixed(2) + '%）' + (reset.length > 0 ? ' · 重置 ' + reset : ''))
          }
          if (quotaPeriods.length === 0) tipLines.push('  该供应商只提供配额查询，不提供金额余额')
        } else if (balanceOk) {
      ...
      const pushMetric = function (label, value) {
        children.push(React.createElement('span', { className: 'tb-sep' }, '·'))
        children.push(React.createElement('span', null, label + ' '))
        children.push(React.createElement('b', null, value))
      }
      if (quotaMode) {
        // 套餐制：单次/本对话是**消耗的额度**，后三个窗口是**剩余额度**
        pushMetric('单次', fmtQuota(lastTurn === null ? 0 : lastTurn.quota))
        pushMetric('本对话', fmtQuota(session.quota))
        for (let index = 0; index < quotaRemaining.length; index += 1) {
          pushMetric(quotaWindowLabel(quotaRemaining[index].label), fmtQuota(quotaRemaining[index].remaining))
        }
      } else {
        pushMetric('单次', lastTurn === null ? '—' : fmtMoney(lastTurn.cost, config.currency))
        pushMetric('本对话', fmtMoney(session.cost, config.currency))
        pushMetric('今日', fmtMoney(today.cost, config.currency))
      }
      if (showBalance && quotaMode !== true) {
        children.push(React.createElement('span', { className: 'tb-sep' }, '·'))
    ...
    function BillingPanel() {
      ...
      const childCost = childList.reduce(function (sum, item) { return sum + item.bucket.cost }, 0)
      const childQuota = childList.reduce(function (sum, item) { return sum + (Number(item.bucket.quota) || 0) }, 0)
      ...
      const submit = function (nextDraft, message) {
        ...
        setBusy(true)
        setNotice('')
        hostCall('billing/save', { sessionId: null, config: fromDraft(nextDraft) }).then(function (next) {
      ...
      const flushStore = function () {
        ...
        setBusy(true)
        setNotice('')
        hostCall('billing/store', { sessionId: null }).then(function (next) {
          if (next !== null && typeof next === 'object') billing.setState(next)
          setBusy(false)
          setNotice('已写入存档')
        }).catch(function (error) {
      ...
      const refreshBalance = function () {
        ...
        setBusy(true)
        setNotice('')
        hostCall('billing/balance', { sessionId: null, force: true }).then(function (next) {
          if (next !== null && typeof next === 'object') billing.setState(next)
          setBusy(false)
          setNotice('余额已刷新')
        }).catch(function (error) {
      ...
      const clearLedger = function () {
        ...
        setBusy(true)
        setNotice('')
        hostCall('billing/reset', { scope: 'all', sessionId: null }).then(function (next) {
          if (next !== null && typeof next === 'object') billing.setState(next)
          setBusy(false)
      ...
      const requestRows = state.rowsAll.map(function (row) {
        return React.createElement('tr', { key: String(row.seq) },
          ...
          React.createElement('td', { className: 'tb-num tb-cost' }, amountText(row.cost, row.quota)))
      })
      ...
      return React.createElement('div', { className: 'tb-page' },
        ...
        quotaMode && quotaRemaining.length > 0
          ? React.createElement('div', null,
            React.createElement('div', { className: 'tb-section-title' }, '套餐额度剩余'),
            cards(quotaRemaining.map(function (item) {
              const reset = String(item.resetAt === undefined ? '' : item.resetAt).replace('T', ' ').slice(0, 16)
              return card(quotaWindowLabel(item.label), fmtQuota(item.remaining),
                '已用 ' + fmtQuota(item.used) + ' / ' + fmtQuota(item.total) + '（' + Number(item.percent).toFixed(2) + '%）'
                  + (reset.length > 0 ? ' · 重置 ' + reset : ''))
            })))
          : null,
          ...
          React.createElement('div', { className: 'tb-section-title' },
            '套餐额度'
              + (balance.planMeta !== undefined && balance.planMeta !== null && typeof balance.planMeta.tier === 'string' && balance.planMeta.tier.length > 0
                ? '（' + balance.planMeta.tier + ' 档）'
                : '')
              + '　来源 ' + String(balance.via === undefined ? '' : balance.via)),
          table([
            { label: '窗口' }, { label: '已用', num: true }, { label: '额度', num: true },
            { label: '剩余', num: true }, { label: '占比', num: true }, { label: '下次重置' },
          ], balance.quotaPeriods.map(function (period, index) {
            const total = Number(period.total) || 0
            const used = Number(period.used) || 0
            return React.createElement('tr', { key: 'quota-' + String(index) },
              React.createElement('td', null, quotaWindowLabel(period.label)),
              React.createElement('td', { className: 'tb-num' }, fmtQuota(used)),
              React.createElement('td', { className: 'tb-num' }, fmtQuota(total)),
              React.createElement('td', { className: 'tb-num' }, fmtQuota(Math.max(0, total - used))),
        ...
        childList.length === 0 ? null : React.createElement('div', null,
          ...
          table([
            ...
            return React.createElement('tr', { key: 'child-' + String(index) },
              ...
              React.createElement('td', { className: 'tb-num' }, amountText(item.bucket.cost, item.bucket.quota)))
          }), '没有子会话')),
