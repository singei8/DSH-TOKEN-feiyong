      ...
      if (showBalance) {
        ...
        if (balanceOk && balanceKind === 'quota') {
          ...
          for (let index = 0; index < quotaPeriods.length; index += 1) {
            ...
            tipLines.push('  ' + String(period.label) + ' 已用 ' + Number(period.used).toFixed(2) + '/' + String(period.total)
      ...
      return React.createElement('div', { className: 'tb-page' },
        ...
        (balance === null || !Array.isArray(balance.quotaPeriods) || balance.quotaPeriods.length === 0) ? null : React.createElement('div', null,
          ...
          table([
            ...
            { label: '占比', num: true }, { label: '下次重置' },
          ], balance.quotaPeriods.map(function (period, index) {
            return React.createElement('tr', { key: 'quota-' + String(index) },
              React.createElement('td', null, String(period.label)),
              React.createElement('td', { className: 'tb-num' }, Number(period.used).toFixed(2)),
              React.createElement('td', { className: 'tb-num' }, String(period.total)),
