...
/** 套餐额度（AFP）的观测状态：用于估算 AFP/Token 比率。 */
const planSeenUsed = {}
let planSeenReady = false
let planSeenSeq = 0
/** 观测到的 AFP/Token 比率；套餐制模型按它折算每次调用的额度消耗。 */
let planQuotaRate = 0
...
function recordCall(meta, usage) {
  ...
  const tokenWeight = (Number(usage.cacheReadTokens) || 0) + (Number(usage.inputTokens) || 0)
    + (Number(usage.cacheWriteTokens) || 0) + (Number(usage.outputTokens) || 0)
  const row = {
    seq: (counter += 1),
    ts: ms,
    day: dayKey(ms),
    ...
    quota: quotaBased === true ? Math.round(tokenWeight * planQuotaRate * 1e6) / 1e6 : 0,
...
function snapshot(sessionId) {
  ...
  return {
    ...
    /** 套餐制（额度）口径：前端据此把 ¥ 换成「单次/本对话额度 + 各窗口剩余」。 */
    quota: {
...
/** 上次额度快照之后记录的 token 量（用于估 AFP/Token 比率）。 */
function tokensSinceSeq(seq) {
  let tokens = 0
  for (let index = 0; index < ledger.length; index += 1) {
    const row = ledger[index]
    if (row.seq <= seq) continue
    tokens += (Number(row.hit) || 0) + (Number(row.miss) || 0) + (Number(row.write) || 0) + (Number(row.out) || 0)
  }
  return tokens
}

/** 用新快照更新 AFP/Token 比率（各窗口增量相同，取最大更稳）。
 *
 * 控制台是**延迟**出账的：某一刻取快照可能看不到任何增量，而这段时间的 token 是真实
 * 消耗掉的。所以只有「真的用掉一次增量」时才推进 token 基准 `planSeenSeq`；
 * 没看到增量就保留基准，把这段 token 攒到下一次增量里一起除 —— 否则会把一段时间的
 * 额度摊到其中一小段 token 上，比率被高估。 */
function observePlanPeriods(periods) {
  ...
  const tokens = tokensSinceSeq(planSeenSeq)
  if (planSeenReady !== true) {
    planSeenReady = true
    planSeenSeq = counter
  } else if (delta > 0 && tokens > 0) {
    planQuotaRate = delta / tokens
    planSeenSeq = counter
    logInfo('plan rate: ' + planQuotaRate.toFixed(8) + ' AFP/token (delta=' + delta.toFixed(6) + ', tokens=' + String(tokens) + ')')
  }
}
