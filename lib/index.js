...
function resolveBalanceProfile(sessionId) {
  ...
  const provider = active === null || active === undefined ? '' : String(active.provider)
  const model = active === null || active === undefined ? '' : String(active.model)
...
function kickBalance(force, profile) {
  ...
  if (balanceLoadingKey === profileKey) return Promise.resolve(balance)
  if (force !== true && balance.profileKey === profileKey && balance.at > 0 && (now - balance.at) < BALANCE_TTL_MS) {
    return Promise.resolve(balance)
  }
  balanceLoadingKey = profileKey
  return fetchBalance(profile).then(function (next) {
    balance = next
    balanceLoadingKey = ''
    return balance
...
function resetBalance() {
  balance = {
    ...
    isAvailable: null, currency: '', total: 0, granted: 0, toppedUp: 0, via: '',
    profileKey: '', kind: 'balance', label: '余额', quotaText: '', providerKey: '', providerLabel: '',
  }
  balanceLoadingKey = ''
}
...
function applyConfig(input) {
  ...
  if (profiles !== null && typeof profiles === 'object' && !Array.isArray(profiles)) {
    ...
    for (let index = 0; index < profileKeys.length; index += 1) {
      ...
      const kind = (entry.kind === 'deepseek' || entry.kind === 'bigmodel' || entry.kind === 'zhipu-quota' || entry.kind === 'auto') ? entry.kind : 'auto'
      nextProfiles[key] = {
        providerKey: key,
